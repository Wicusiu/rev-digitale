...
import { IUserService, Credentials, AuthCredentials } from 'app/business/user/IUserService';
...
export class UserService extends JsonServiceBase<User> implements IUserService {
  ...
  authSignIn: (credentials: AuthCredentials) => Promise<IUser> = (credentials) => {
    const params = UserApiFetchParamCreator().usersAuthSigninPost(credentials);
